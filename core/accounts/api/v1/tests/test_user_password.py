...
@pytest.mark.django_db
def test_change_password_with_token_with_invalid_data(
...
@pytest.mark.django_db
def test_change_password_with_jwt_token_with_invalid_data(
    client, jwt_access_token, current_password, new_password, new_password2
):
    url = reverse("account:api-v1:change-password")
    data = {
        "current_password": current_password,
...
def test_reset_password_confirm_with_valid_token_with_valid_data(client, email_token):
    url = reverse(
        "account:api-v1:reset-password-confirm", kwargs={"token": email_token}
    )
    data = {"new_password": "testpass321", "new_password2": "testpass321"}
    response = client.put(path=url, data=data)
    response_data = response.data.get("Message")
    assert response.status_code == 200
    ...
    # testing if password was changed or not using simple authenticate.
    user = authenticate(
        response.request, username="test@mail.com", password="testpass321"
    )
    assert user is not None
...
@pytest.mark.django_db
def test_reset_password_confirm_with_valid_token_with_invalid_data(
    client, email_token, new_password, new_password2
):
    url = reverse(
        "account:api-v1:reset-password-confirm", kwargs={"token": email_token}
    )
    data = {"new_password": new_password, "new_password2": new_password2}
    response = client.put(path=url, data=data)
    assert response.status_code == 400
...
def test_reset_password_confirm_with_invalid_token(client, invalid_email_token):
    # there is no user with this email.
    url = reverse(
        "account:api-v1:reset-password-confirm", kwargs={"token": invalid_email_token}
    )
    data = {"new_password": "testtest123", "new_password2": "testtest123"}
    response = client.put(path=url, data=data)
    response_data = response.data.get("detail")
