...
def test_user_create_task_with_valid_data(client, user1, title, description, status):
    ...
    url = reverse('todo:api-v4:task-list-create')
    ...
    assert response_data['status'] == task.get_status_display() == (STATUS_DICT[status] if status is not None else "Todo")
...
@pytest.mark.django_db
def test_user2_create_task_with_exact_title_that_user1_has_task(client, user2_with_tasks, user1):
...
def test_user_get_task(client, user1_with_tasks):
    ...
    for task in user_tasks:
        url = reverse('todo:api-v4:task-detail', kwargs={'slug':task.slug})
        response = client.get(path=url)
        assert response.status_code == 200

@pytest.mark.django_db
def test_unauthenticated_user_get_task(client, user1):
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'first-task-for-user1'})
    response = client.get(path=url)
    response_data = response.data.get('detail')
    assert response.status_code == 401
    assert response_data == 'Authentication credentials were not provided.'
...
def test_user_get_unavailable_task(client, user1_with_tasks):
    user = user1_with_tasks
    client.force_authenticate(user)
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'not-available'})
    response = client.get(path=url)
...
@pytest.mark.django_db
def test_user_update_task_partially_with_valid_data(client, user1_with_tasks, title, description, status):
    user = user1_with_tasks
    client.force_authenticate(user)
...
@pytest.mark.django_db
def test_user_update_task_partially_with_valid_data_but_duplicate_title(client, user1_with_tasks):
...
@pytest.mark.django_db
def test_unauthenticated_user_update_task_partially_with_valid_data(client, user1_with_tasks):
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'edit-test'})
...
@pytest.mark.django_db
def test_unauthenticated_user_update_task_partially_with_valid_data_but_duplicate_title(client, user1_with_tasks):
    user = user1_with_tasks
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'edit-test'})
    data = {'description': 'edited description for user1'}
    response = client.patch(path=url, data=data)
...
@pytest.mark.django_db
def test_unauthenticated_user_partially_update_unavailable_task(client, user1_with_tasks):
    user = user1_with_tasks
    number_of_tasks_for_this_user = len(user.tasks.all())
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'unavailable'})
    data = {'title': 'not available'}
...
@pytest.mark.django_db
def test_user_update_task_with_valid_data(client, user1_with_tasks, title, description, status):
    user = user1_with_tasks
    client.force_authenticate(user)
    task = Task.objects.get(title='edit test')
    number_of_tasks_for_this_user = len(user.tasks.all())
...
@pytest.mark.django_db
def test_unauthenticated_user_update_task_with_valid_data_but_duplicate_title(client, user1_with_tasks):
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'edit-test'})
    data = {'description': 'edited description for user1'}
    response = client.put(path=url, data=data)
    response_data = response.data.get('detail')
...
def test_user_delete_task(client, user1_with_tasks):
    ...
    url = reverse('todo:api-v4:task-detail', kwargs={'slug':'delete-test'})
